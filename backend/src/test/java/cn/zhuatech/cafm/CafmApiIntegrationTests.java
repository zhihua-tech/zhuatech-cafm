 ...
 */
@SpringBootTest @AutoConfigureMockMvc class CafmApiIntegrationTests {
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanReadShopfloorDashboard()throws Exception{mvc.perform(get("/api/shopfloor/dashboard").header("Authorization","Bearer "+operatorToken)).andExpect(status().isOk()).andExpect(jsonPath("$.success").value(true)).andExpect(jsonPath("$.data.metrics[0].label").value("今日服务处置"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
     ...
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void operatorCanSubmitProductionReport()throws Exception{mvc.perform(post("/api/shopfloor/work-orders/1/reports").header("Authorization","Bearer "+operatorToken).contentType(MediaType.APPLICATION_JSON).content("{\"operationName\":\"服务处置执行\",\"goodQty\":2,\"defectQty\":1,\"remark\":\"数据完整\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.message").value("反馈提交成功")).andExpect(jsonPath("$.data.completedQty").value(18));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
    @Test void plannerCanAnalyzeSpaceUtilization()throws Exception{mvc.perform(post("/api/admin/space-utilization").header("Authorization","Bearer "+plannerToken).contentType(MediaType.APPLICATION_JSON).content("{\"spaceName\":\"研发办公区\",\"areaSqm\":2000,\"seats\":200,\"averageOccupancyPercent\":40,\"peakOccupancyPercent\":60,\"monthlyCost\":200000,\"flexibleSeats\":20}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.unusedSeats").value(120)).andExpect(jsonPath("$.data.annualSavingsPotential").value(864000)).andExpect(jsonPath("$.data.decision").value("CONSOLIDATE"));}
    /**
     * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
     */
